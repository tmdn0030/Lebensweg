    ...
    float GetSpeedMultiplier(float distance)
    {
        ...
        for (int i = 0; i < sections.Count - 1; i++)
        {
            ...
            if (distance >= current.startDistance && distance <= next.startDistance)
            {
                ...
                /*
                var curve = speedProfile.GetEasingCurveForDistance(distance);
                bool isReversing = current.speedMultiplier > next.speedMultiplier;
                float adjustedT = isReversing ? 1f - t : t;
                t = curve.Evaluate(adjustedT);
                */
